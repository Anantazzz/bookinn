            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
                <div>
                    <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-2 flex items-center gap-2 sm:gap-3">
                        ...
                        <span class="break-words">Manajemen Kamar</span>
                    </h1>
                    <p class="text-sm sm:text-base text-gray-600">Kelola data kamar hotel dengan mudah dan efisien</p>
                </div>
                <button 
        ...
        <div class="lg:hidden space-y-4">
                ...
                <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                    ...
                    <div class="relative">
                        ...
                        <div class="absolute top-3 right-3">
                            @if($kamar->status == 'tersedia')
                                <span class="inline-flex items-center gap-1 bg-green-500 text-white px-3 py-1.5 rounded-full font-semibold text-xs shadow-lg">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3" fill="currentColor" viewBox="0 0 16 16">
                                        <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zm-3.97-3.03a.75.75 0 0 0-1.08.022L7.477 9.417 5.384 7.323a.75.75 0 0 0-1.06 1.06L6.97 11.03a.75.75 0 0 0 1.079-.02l3.992-4.99a.75.75 0 0 0-.01-1.05z"/>
                                    </svg>
                                    Tersedia
                            ...
                            @else
                                <span class="inline-flex items-center gap-1 bg-orange-500 text-white px-3 py-1.5 rounded-full font-semibold text-xs shadow-lg">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3" fill="currentColor" viewBox="0 0 16 16">
                                        <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/>
                                        <path d="M8 4a.5.5 0 0 1 .5.5v3h3a.5.5 0 0 1 0 1h-3v3a.5.5 0 0 1-1 0v-3h-3a.5.5 0 0 1 0-1h3v-3A.5.5 0 0 1 8 4z"/>
                                    </svg>
                    ...
                    <div class="p-4">
                        {{-- Nomor Kamar & Hotel --}}
                        <div class="mb-3">
                            <h3 class="text-xl font-bold text-gray-900 mb-1">Kamar {{ $kamar->nomor_kamar }}</h3>
                            <p class="text-sm text-gray-600">{{ $kamar->hotel->nama_hotel ?? '-' }}</p>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-3 mb-4">
                            <div class="bg-gray-50 rounded-lg p-3">
                                ...
                                <p class="text-sm font-semibold text-gray-900">{{ $kamar->tipeKamar->nama_tipe ?? '-' }}</p>
                            </div>
                            
                            <div class="bg-gray-50 rounded-lg p-3">
                                <p class="text-xs text-gray-500 mb-1">Harga</p>
                                <p class="text-sm font-semibold text-blue-600">Rp {{ number_format($kamar->harga, 0, ',', '.') }}</p>
                            </div>
                            ...
                            <div class="bg-gray-50 rounded-lg p-3">
                                <p class="text-xs text-gray-500 mb-1">Jumlah Bed</p>
                                <p class="text-sm font-semibold text-gray-900">{{ $kamar->jumlah_bed }} bed</p>
                            </div>

                            <div class="bg-gray-50 rounded-lg p-3 col-span-2">
                                <p class="text-xs text-gray-500 mb-1">Fasilitas Internet</p>
                                @if($kamar->internet)
                                ...
                                @else
                                    <p class="text-sm font-semibold text-gray-400">Tidak ada WiFi</p>
                                @endif
                            </div>
                        </div>
            ...
            @empty
                <div class="bg-white rounded-2xl shadow-lg p-8 text-center">
                    ...
                    <p class="text-gray-500 font-semibold text-lg mb-1">Belum ada data kamar</p>
                    <p class="text-gray-400 text-sm">Klik tombol "Tambah Kamar" untuk menambahkan kamar baru</p>
                </div>
            @endforelse
        </div>
...
{{-- ================== MODAL TAMBAH KAMAR ================== --}}
<div id="addKamarModal" class="hidden fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50 p-3 sm:p-4 animate-fade-in">
  <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl relative animate-slide-up max-h-[90vh] overflow-y-auto">
    <div class="bg-gradient-to-r from-blue-600 to-purple-600 rounded-t-2xl p-4 sm:p-6 sticky top-0 z-10">
      <h2 class="text-xl sm:text-2xl font-bold text-white flex items-center gap-2 sm:gap-3">
        ...
        <span class="break-words">Tambah Kamar Baru</span>
      </h2>
    </div>

    <form action="{{ route('admin.kamars.store') }}" method="POST" enctype="multipart/form-data" class="p-4 sm:p-6">
      @csrf

      <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
        <div>
          <label class="block text-xs sm:text-sm font-semibold text-gray-700 mb-2">Hotel</label>
          <select name="hotel_id" class="w-full border-2 border-gray-200 rounded-xl px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" required>
        ...
        <div>
          <label class="block text-xs sm:text-sm font-semibold text-gray-700 mb-2">Tipe Kamar</label>
          <select name="tipe_kamar_id" class="w-full border-2 border-gray-200 rounded-xl px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" required>
        ...
        <div>
          <label class="block text-xs sm:text-sm font-semibold text-gray-700 mb-2">Nomor Kamar</label>
          <input type="text" name="nomor_kamar" class="w-full border-2 border-gray-200 rounded-xl px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" placeholder="Contoh: 101" required>
        </div>

        <div>
          <label class="block text-xs sm:text-sm font-semibold text-gray-700 mb-2">Harga</label>
          <input type="number" name="harga" class="w-full border-2 border-gray-200 rounded-xl px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" placeholder="Contoh: 500000" required>
        </div>

        <div>
          <label class="block text-xs sm:text-sm font-semibold text-gray-700 mb-2">Kapasitas</label>
          <input type="number" name="kapasitas" class="w-full border-2 border-gray-200 rounded-xl px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" placeholder="Jumlah orang" required>
        </div>

        <div>
          <label class="block text-xs sm:text-sm font-semibold text-gray-700 mb-2">Jumlah Bed</label>
          <input type="number" name="jumlah_bed" class="w-full border-2 border-gray-200 rounded-xl px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" placeholder="Jumlah tempat tidur" required>
        </div>

        <div>
          <label class="block text-xs sm:text-sm font-semibold text-gray-700 mb-2">Internet</label>
          <select name="internet" class="w-full border-2 border-gray-200 rounded-xl px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all">
        ...
        <div>
          ...
          <select name="status" class="w-full border-2 border-gray-200 rounded-xl px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all">
        ...
        <div class="sm:col-span-2">
          <label class="block text-xs sm:text-sm font-semibold text-gray-700 mb-2">Gambar Kamar</label>
      ...
      <div class="flex flex-col sm:flex-row gap-2 sm:gap-3 mt-4 sm:mt-6">
        ...
        <button type="submit" class="flex-1 px-4 sm:px-6 py-2.5 sm:py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl hover:shadow-lg hover:scale-105 transition-all font-semibold text-sm sm:text-base">Simpan Kamar</button>
      </div>
    </form>
  </div>
</div>
...
{{-- ================== MODAL EDIT KAMAR ================== --}}
<div id="editKamarModal" class="hidden fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50 p-3 sm:p-4 animate-fade-in">
  <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl relative animate-slide-up max-h-[90vh] overflow-y-auto">
    <div class="bg-gradient-to-r from-amber-500 to-orange-500 rounded-t-2xl p-4 sm:p-6 sticky top-0 z-10">
      <h2 class="text-xl sm:text-2xl font-bold text-white flex items-center gap-2 sm:gap-3">
        ...
        <span class="break-words">Edit Kamar</span>
      </h2>
    </div>

    <form id="editKamarForm" method="POST" enctype="multipart/form-data" class="p-4 sm:p-6">
      @csrf
      @method('PUT')
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
        <div>
          <label class="block text-xs sm:text-sm font-semibold text-gray-700 mb-2">Hotel</label>
          <select id="edit_hotel_id" name="hotel_id" class="w-full border-2 border-gray-200 rounded-xl px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all" required>
            @foreach(\App\Models\Hotel::all() as $hotel)
              <option value="{{ $hotel->id }}">{{ $hotel->nama_hotel }}</option>
            @endforeach
          </select>
        ...
        <div>
          <label class="block text-xs sm:text-sm font-semibold text-gray-700 mb-2">Tipe Kamar</label>
          <select id="edit_tipe_kamar_id" name="tipe_kamar_id" class="w-full border-2 border-gray-200 rounded-xl px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all" required>
            @foreach(\App\Models\TipeKamar::all() as $tipe)
              <option value="{{ $tipe->id }}">{{ $tipe->nama_tipe }}</option>
            @endforeach
          </select>
        ...
        <div>
          <label class="block text-xs sm:text-sm font-semibold text-gray-700 mb-2">Nomor Kamar</label>
          <input type="text" id="edit_nomor_kamar" name="nomor_kamar" class="w-full border-2 border-gray-200 rounded-xl px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all" required>
        </div>

        <div>
          <label class="block text-xs sm:text-sm font-semibold text-gray-700 mb-2">Harga</label>
          <input type="number" id="edit_harga" name="harga" class="w-full border-2 border-gray-200 rounded-xl px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all" required>
        </div>

        <div>
          <label class="block text-xs sm:text-sm font-semibold text-gray-700 mb-2">Kapasitas</label>
          <input type="number" id="edit_kapasitas" name="kapasitas" class="w-full border-2 border-gray-200 rounded-xl px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all" required>
        </div>

        <div>
          <label class="block text-xs sm:text-sm font-semibold text-gray-700 mb-2">Jumlah Bed</label>
          <input type="number" id="edit_bed" name="jumlah_bed" class="w-full border-2 border-gray-200 rounded-xl px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all" required>
        </div>

        <div>
          <label class="block text-xs sm:text-sm font-semibold text-gray-700 mb-2">Internet</label>
          <select id="edit_internet" name="internet" class="w-full border-2 border-gray-200 rounded-xl px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all">
            <option value="1">Ada</option>
            <option value="0">Tidak</option>
          </select>
        </div>
        ...
        <div>
          <label class="block text-xs sm:text-sm font-semibold text-gray-700 mb-2">Status</label>
          <select id="edit_status" name="status" class="w-full border-2 border-gray-200 rounded-xl px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all">
            <option value="tersedia">Tersedia</option>
            <option value="booking">Booking</option>
          </select>
        </div>
        ...
        <div class="sm:col-span-2">
          ...
          <div id="preview_gambar_container" class="mt-3 hidden">
            <img id="preview_gambar" src="" class="w-24 h-24 sm:w-32 sm:h-32 object-cover rounded-xl shadow-md">
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-col sm:flex-row gap-2 sm:gap-3 mt-4 sm:mt-6">
        ...
        <button type="submit" class="flex-1 px-4 sm:px-6 py-2.5 sm:py-3 bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-xl hover:shadow-lg hover:scale-105 transition-all font-semibold text-sm sm:text-base">Update Kamar</button>
      </div>
    </form>
  </div>
</div>
...
document.getElementById('addKamarModal')?.addEventListener('click', function(e) {
  if (e.target === this) {
    closeAddModal();
  }
});

document.getElementById('editKamarModal')?.addEventListener('click', function(e) {
  if (e.target === this) {
    closeEditModal();
  }
});
